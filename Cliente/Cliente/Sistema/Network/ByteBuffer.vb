...
Public Class ByteBuffer
    ...
    Dim buffUpdated As Boolean = False
    ...
    ''' </summary>
    Public Sub New(ByVal PacketNum As Long)
        Buff = New List(Of Byte)
        readpos = 0
        WriteLong(PacketNum)
    End Sub
    ...
    Public Sub WriteString(ByVal Input As String)
        ...
        If Input.Length = 0 Then Input = " "
        Dim bytes() As Byte = Encoding.Unicode.GetBytes(Input)
        WriteInteger(bytes.Length)
        WriteBytes(bytes)
    End Sub

    Public Function ReadString(Optional ByVal Peek As Boolean = True) As String
        Dim Len As Long = ReadInteger(True)
        Dim bytes() As Byte = ReadBytes(Len)
        Dim ret As String = Encoding.Unicode.GetString(bytes)
        ...
        If buffUpdated Then
            ...
            buffUpdated = False
        End If
        ret = Encoding.ASCII.GetString(readBuff, readpos, Len)
